...
?>
	<li class="panel">
        <span class="title-plan stripe-payment" data-type="stripe">
            ...
            <span><?php _e("Send your payment to our stripe account", ET_DOMAIN); ?></span>
        </span>
        <!-- <a href="#" class="btn btn-submit-price-plan other-payment" data-type="stripe"><?php _e("Select", ET_DOMAIN); ?></a> -->
       <!--  <a data-toggle="collapse" data-type="stripe" data-parent="#fre-payment-accordion" href="#fre-payment-stripe" class="btn collapsed other-payment"><?php _e("Select", ET_DOMAIN); ?></a> -->
...
function ae_stripe_script() {
    if (is_page_template('page-post-place.php') || is_page_template('page-submit-project.php')) {
        ...
        wp_enqueue_script('ae_stripe', plugin_dir_url(__FILE__) . '/assets/stripe.js', array(
            ...
            'backbone',
            'appengine'
        ) , FSTRIPE_VERSION , true);
        wp_enqueue_style('ae_stripe', plugin_dir_url(__FILE__) . '/assets/stripe.css', array() , FSTRIPE_VERSION );
...
add_filter('ae_setup_payment', 'ae_stripe_setup_payment', 10, 3); // a filter of et-setup-payment
/**
