...
class WPF_Views_User_Registration_Integration {

	function __construct() {
		//Update Entry Author ID to created user id so that user can edit this entry later
		add_action( 'wpforms_user_registered', array( $this, 'update_entry_author' ), 12, 4 );

		// Disable User registration when editing entry
		add_filter( "wpforms_process_before_form_data", array( $this, 'maybe_disable_user_registration' ), 12, 2 );

		// Update user registration data after entry is edited
		add_action( 'wpf-views/edit-entry/entry-updated',  array( $this, 'update_user_meta' ), 10, 3 );
	 ...
	 * @return void
	 */
	public function update_entry_author( $user_id, $fields, $form_data, $user_data ) {
		if ( ! empty( $_POST['wpforms']['entry_id'] ) ) {
			WPF_Views_Edit_Entry_Db()->update( $_POST['wpforms']['entry_id'], array( 'user_id'=>$user_id ), '', 'edit_entry' );
		}
	}
	 ...
	 * Disable User registration when editing entry
	 *
	 * @param unknown $form_data Form Data
	 * @param unknown $entry     Form submission raw data ($_POST).
	 * @return void
	 */
	public function maybe_disable_user_registration( $form_data, $entry ) {
...
new WPF_Views_User_Registration_Integration();
