...
final class WSAL_AlertManager {
	...
	protected function Log( $event_id, $event_data = array() ) {
		...
		// certain mate data needs to be truncated because Action Scheduler has a limit on the size of the task argument
		// of 8000 characters
		foreach ( $event_data as $meta_key => &$meta_value ) {
			if ( in_array( $meta_key, array( 'MetaValue', 'MetaValueOld', 'MetaValueNew' ) ) ) {
				...
				$meta_value = mb_strlen( $meta_value ) > 50 ? ( mb_substr( $meta_value, 0, 50 ) . '...' ) : $meta_value;
			}
		}
