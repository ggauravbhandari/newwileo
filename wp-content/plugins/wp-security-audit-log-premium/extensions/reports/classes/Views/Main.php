...
class WSAL_Rep_Views_Main extends WSAL_AbstractView {
	...
	public function AjaxGenerateReport() {
		...
		$report = $this->_plugin->reporting_util->GenerateReport( $filters, false );


		// Append to the JSON file.
		$this->_plugin->reporting_util->generateReportJsonFile( $report );
		...
		if ( null == $response[0] ) {
			...
			if ( null == $response[0] ) {
				$response[1] = $this->_plugin->reporting_util->downloadReportFile();
	...
	public function AjaxSendPeriodicReport() {
		...
		$limit       = $_POST['limit'];
		$last_date   = $this->_plugin->reporting_util->sendNowPeriodic( $report_name, $next_date, $limit );
	...
	public function AjaxCheckArchiveMatch() {
		...
		if ( $this->_plugin->settings()->IsArchivingEnabled() ) {
			$filters = $_POST['filters'];
			$this->_plugin->settings()->SwitchToArchiveDB();
			$response = $this->_plugin->reporting_util->IsMatchingReportCriteria( $filters );
		}
		echo json_encode( $response );
		exit;
	}
	...
	public function AjaxSummaryUniqueIPs() {
		$response = false;
		$filters  = $_POST['filters'];
		$response = $this->_plugin->reporting_util->StatisticsUniqueIPS( $filters );
		echo json_encode( $response );
		exit;
	}
	...
	public function SavePeriodicReport( $post_data ) {
		if ( isset( $post_data ) ) {
			$wsalCommon      = $this->_plugin->reporting_util;
			...
			if ( ! empty( $post_data['alert_codes']['groups'] ) ) {
				foreach ( $post_data['alert_codes']['groups'] as $key => $group ) {
					$_codes            = $this->_plugin->reporting_util->GetCodesByGroup( $group );
			...
			$this->_plugin->reporting_util->AddGlobalSetting( $optName, $data );
		}
	}
	...
	private function generateStatisticsReport( $filters ) {
		...
		if ( isset( $_POST['include-archive'] ) ) {
			$this->_plugin->reporting_util->AddGlobalSetting( 'include-archive', true );
		} else {
		    $this->_plugin->reporting_util->DeleteGlobalSetting( 'include-archive' );
		}
		?>
		<script type="text/javascript">
	...
	public function Render() {
		...
		if ( ! is_wp_error( $wpsalRepUploadsDir ) && $this->_plugin->reporting_util->CheckDirectory( $wpsalRepUploadsDir ) ) {
