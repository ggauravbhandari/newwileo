...
class WSAL_Rep_Common {
	...
	public function GenerateReport( array $filters, $validate = true ) {
		...
		$_codes = $this->GetCodesByGroups( $actual_filters['alert_codes_groups'], $actual_filters['alert_codes_alerts'] );
		if ( ! $_codes ) {
			return false;
		}
		 ...
		 */
		$_siteId = $actual_filters['sites'] ? "'" . implode( ',', $actual_filters['sites'] ) . "'" : 'null';
		$_userId = $actual_filters['users'] ? "'" . implode( ',', $actual_filters['users'] ) . "'" : 'null';

		$_post_types = 'null';
		if ( $actual_filters['alert_codes_post_types'] ) {
			$_post_types = array();
			foreach ( $actual_filters['alert_codes_post_types'] as $post_type ) {
		...
		$_post_statuses = 'null';
		if ( $actual_filters['alert_codes_post_statuses'] ) {
			$_post_statuses = array();
			foreach ( $actual_filters['alert_codes_post_statuses'] as $post_status ) {
		...
		$_objects = 'null';
		if ( $actual_filters['objects'] ) {
			$_objects = array();
			foreach ( $actual_filters['objects'] as $object ) {
		...
		$_event_types = 'null';
		if ( $actual_filters['event-types'] ) {
			$_event_types = array();
			foreach ( $actual_filters['event-types'] as $event_type ) {
		...
		$_roleName = 'null';
		if ( $actual_filters['roles'] ) {
			$_roleName = array();
			foreach ( $actual_filters['roles'] as $k => $role ) {
				array_push( $_roleName, esc_sql( '(' . preg_quote( $role ) . ')' ) );
			}
			$_roleName = "'" . implode( '|', $_roleName ) . "'";
		}
		$_ipAddress = $actual_filters['ip-addresses'] ? "'" . implode( ',', $actual_filters['ip-addresses'] ) . "'" : 'null';
		...
		if ( $actual_filters['date_range_start'] ) {
			$start_datetime  = DateTime::createFromFormat( $this->_dateFormat . ' H:i:s', $actual_filters['date_range_start'] . ' 00:00:00' );
			$_startTimestamp = $start_datetime->format( 'U' );
		}
		if ( $actual_filters['date_range_end'] ) {
			$end_datetime  = DateTime::createFromFormat( $this->_dateFormat . ' H:i:s', $actual_filters['date_range_end'] . ' 23:59:59' );
			$_endTimestamp = $end_datetime->format( 'U' );
		}
	...
	private function _getAlertDetails( $report_format, $entryId, $alertId, $siteId, $createdOn, $object, $event_type, $userId = null, $roles = null, $ip = '', $ua = '' ) {
		...
		return array(
			...
			'message'    => $this->ko->GetAlert()->GetMessage( $this->ko->GetMetaArray(), $this->ko->_cachedmessage, $entryId, $messageContext ),
			'user_id'    => $userId,
			'user_name'  => $username,
			'role'       => $role,
			'user_ip'    => $ip,
