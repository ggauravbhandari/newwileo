...
class Builder {
	...
	public function panel_content( $instance ) {
		...
		if ( ! wpforms_save_resume()->is_enabled( $instance->form_data ) ) {
			...
			printf(
				...
				'https://wpforms.com/docs/how-to-install-and-use-the-save-and-resume-addon-with-wpforms/'
			);
			echo '</div>';
		}
		...
		wpforms_panel_field(
			...
			'save_resume_enable',
			$instance->form_data,
			esc_html__( 'Enable Save and Resume', 'wpforms-save-resume' )
		);
		...
		wpforms_panel_field(
			...
			$instance->form_data,
			esc_html__( 'Link Text', 'wpforms-save-resume' ),
			[
				'tooltip' => esc_html__( 'This text is displayed next to the submit button, and allows users to save their progress.', 'wpforms-save-resume' ),
		...
		$disclaimer = wpforms_panel_field(
			...
			$instance->form_data,
			esc_html__( 'Enable Disclaimer Page', 'wpforms-save-resume' ),
			[],
			false
		);
		...
		wpforms_panel_fields_group(
			$disclaimer,
			[
				'description' => esc_html__( 'This page is displayed before the user actually commits to saving their progress.', 'wpforms-save-resume' ),
				'title'       => esc_html__( 'Disclaimer Page', 'wpforms-save-resume' ),
			]
		);
		...
		wpforms_panel_field(
			...
			$instance->form_data,
			esc_html__( 'Display Message', 'wpforms-save-resume' ),
			[
				'tinymce'       => [
					'editor_height' => 175,
				],
		...
		wpforms_panel_fields_group(
			$confirmation,
			[
				'description' => esc_html__( 'This page is displayed once the user clicks the Save and Resume link and provides instructions for resuming.', 'wpforms-save-resume' ),
				'title'       => esc_html__( 'Confirmation Page', 'wpforms-save-resume' ),
			]
		);
		...
		wpforms_panel_field(
			...
			$instance->form_data,
			esc_html__( 'Enable Resume Link', 'wpforms-save-resume' ),
			[
				'tooltip' => esc_html__( 'Allow the user to copy a link, which they can paste in their address bar to resume later.', 'wpforms-save-resume' ),
		...
		wpforms_panel_field(
			...
			$instance->form_data,
			esc_html__( 'Enable Email Notification', 'wpforms-save-resume' ),
			[
				'tooltip' => esc_html__( 'Allow the user to receive an email with a link to resume later.', 'wpforms-save-resume' ),
				'default' => ! isset( $instance->form_data['settings']['save_resume_enable'] ) ? '1' : 0,
			]
		);
		...
		$notification_message = wpforms_panel_field(
			...
			$instance->form_data,
			esc_html__( 'Email Notification', 'wpforms-save-resume' ),
			[
		...
		wpforms_panel_fields_group(
			...
			[
				'description' => esc_html__( 'When the user chooses to email a link, they are shown the following success message and receive an email with the link.', 'wpforms-save-resume' ),
				'title'       => esc_html__( 'Email Settings', 'wpforms-save-resume' ),
			]
		);
	...
	public function builder_strings( $strings ) {
		...
		$strings['save_resume_email_link_settings_required'] = esc_html__( 'One option must be enabled so that your visitor can resume their entry', 'wpforms-save-resume' );
