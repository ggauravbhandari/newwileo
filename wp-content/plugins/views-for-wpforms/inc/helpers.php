...
function wpforms_views_get_submissions( $args ) {
	...
	if ( ! empty( $args['filter'] ) ) {
		foreach ( $args['filter'] as $filter ) {
			...
			if ( $field_id !== 'all_fields' ) {
				...
				$value = apply_filters( 'wpforms_views_filter_value', $filter['value'], $field_id, $args['view_id'], $form_id );
				$value = apply_filters( "wpf-views/filter/{$field_type}-value", $value, $field_id, $args['view_id'], $form_id );
				...
				// For Date Time fileds cast column & value to mysql date objects
				//https://stackoverflow.com/questions/32220641/varchar-date-time-comparison-issue
				if ( $field_type == 'date-time' ) {
					$col_name = "`m$i`.`value`";
					$date_col_string = wpf_views_cast_to_mysql_date( $form_data, $field_id, $col_name );
					$value = "'" . $value . "'";
					$date_value_string = wpf_views_cast_to_mysql_date( $form_data, $field_id, $value );

					$where[] = "(`m$i`.`field_id` = '$field_id' AND ($date_col_string) $comparison_operator $date_value_string)";
	...
	if ( ! empty( $args['entry_date'] ) ) {
		foreach ( $args['entry_date'] as  $entry_date ) {
			$date = WPForms_Views_MergeTags()->replace( $entry_date['value'] );
			...
			$comparison_operator = $entry_date['compare'];
...
function wpf_views_cast_to_mysql_date( $form_data, $field_id, $col_name ) {
