<?php
	...
	$user_role	=	ae_user_role($user->ID);
		...
		<div class="et-act">
			...
			<select name="role" class="role-change regular-input" >
				<?php foreach ( $role_names as $role_name => $role_label ) {
				    if($role_name === 'freelancer' || $role_name === 'employer' ) {
				        $role_label = 'Client';
                        ...
                        if ($role_name == $user_role)
                            echo '<option data-cu="' . $user_role . '" value="' . $role_name . '" selected="selected">' . $role_label . '</option>';
                        else
