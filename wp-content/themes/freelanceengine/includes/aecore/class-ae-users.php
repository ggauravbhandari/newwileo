...
class AE_Users {
    ...
    public function convert( $user ) {
        ...
        if ( current_user_can( 'edit_users' ) && isset( $user->roles ) ) {
            ...
            $result->role  = ae_user_role( $user->ID );
    ...
    public function get_avatar( $id_or_email, $size, $default = '' ) {
        ...
        if ( $id_or_email ) {
            ...
            if ( is_ssl() ) {
                ...
                $host = 'https://www.wileo.com/wp-content/uploads/2021/12/image-20211012-032834.png';
    ...
    public function forgot( $user_data ) {
        ...
        if ( empty( $user_data['user_login'] ) ) {
            ...
            if ( empty( $user_data ) ) {
                ...
                $errors->add( 'invalid_email', __( 'Your email has not been found in our database.', ET_DOMAIN ) );
    ...
    function check_activation_key( $key, $login ) {
        ...
        //return new WP_Error( 'invalid_key', __( $key, ET_DOMAIN ) );
        ...
        $user = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->users WHERE user_activation_key = %s AND user_login = %s",   $key , esc_sql( $login ) ) );

        if ( empty( $user ) ) {

            return new WP_Error( 'invalid_key', __( 'Invalid Activation Key  here.', ET_DOMAIN ) );
