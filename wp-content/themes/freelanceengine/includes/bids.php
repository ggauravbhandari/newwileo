...
class Fre_BidAction extends AE_PostAction {
	...
	function bid_sync() {
		...
		if ( is_wp_error( $result ) ) {
			...
			// $term_obj_name = wp_get_post_terms($_REQUEST['ID'], 'project_type',  array("fields" => "all"));
			// if(!empty($term_obj_name[0]) && $term_obj_name[0]->slug == 'time-based'){
			// 	$message = __( "Update time based offer successful.", ET_DOMAIN );
			// }else if(!empty($term_obj_name[0]) && $term_obj_name[0]->slug == 'projects-panels'){
			// 	$message = __( "Update your express intrest successful.", ET_DOMAIN );
			// }else{
				$message = __( "Update offer successful.", ET_DOMAIN );
			// }
			
			if ( $method == 'create' ) {

				// if((!empty($_REQUEST['hour_per_week']) ) || !empty($_POST['hour_per_week'])){
					
				// 	add_post_meta($result->ID, 'time_based_apply_offer', $_REQUEST['hour_per_week']);
				...
				$message = __( "Create offer successful.", ET_DOMAIN );
			}
			wp_send_json( array(
				'success' => true,
				'msg'     => $message
	...
	*/
	function fre_check_before_insert_bid( $args ) {
		...
		echo'</pre>';
		die;*/
		$term_obj_name = wp_get_post_terms($args['post_parent'], 'project_type',  array("fields" => "all"));
		...
		*/
		...
		/* Add code by ocean 25-05-2022 to apply condition as per project type */
		if(!empty($term_obj_name[0]) && $term_obj_name[0]->slug == 'time-based'){
				//$message = __( "Update time based offer successful.", ET_DOMAIN );
			if ( in_array( 'hour_per_week', $bid_required_field ) && ( ! isset( $args['hour_per_week'] ) || empty( $args['hour_per_week'] ) ) ) {
				return new WP_Error( 'empty_time', __( 'You have to set the hours per week.', ET_DOMAIN ) );
			}
		}else if(!empty($term_obj_name[0]) && $term_obj_name[0]->slug == 'projects-panels'){
			//$message = __( "Update your express intrest successful.", ET_DOMAIN );
		}else{
		...
		if(!empty($term_obj_name[0]) && $term_obj_name[0]->slug == 'time-based'){
			...
			if ( ( in_array( 'bid_budget', $bid_required_field ) && ! is_numeric( $args['bid_budget'] ) ) ) {
				return new WP_Error( 'invalid_input', __( 'Please enter a valid number in budget or offer time', ET_DOMAIN ) );
			}
		}else if(!empty($term_obj_name[0]) && $term_obj_name[0]->slug == 'projects-panels'){
			//$message = __( "Update your express intrest successful.", ET_DOMAIN );
		}else{
		...
		if ( ! $user_ID ) {
			return new WP_Error( 'no_permission', __( 'Please login to offer a project', ET_DOMAIN ) );
		}
		...
		if ( fre_has_bid( $project_id ) ) {
			return new WP_Error( 200, __( 'You have offer on this project', ET_DOMAIN ) );
		}

		$post_author = (int) get_post_field( 'post_author', $project_id, 'display' );

		if ( $user_ID == $post_author ) {
			return new WP_Error( 200, __( 'You can\'t offer on your project', ET_DOMAIN ) );
		}

		// check role to bid project
		$role = ae_user_role();
		if ( ! fre_share_role() && $role != FREELANCER ) {
			return new WP_Error( 200, __( 'You have to be a freelancer to offer a project.', ET_DOMAIN ) );
		}
	...
	*/
	function fre_update_after_bidding( $bid_id ) {
		...
		$getRes = fre_getProjectPostType($project_id);
		
		if(!empty($getRes) && $getRes->slug == 'time-based'){
			if((!empty($_REQUEST['hour_per_week']) ) || !empty($_POST['hour_per_week'])){
					
				add_post_meta($bid_id, 'time_based_apply_offer', $_REQUEST['hour_per_week']);
		...
		wp_send_json( array(
			'success' => true,
			'msg'     => __( 'You have successfully offer.', ET_DOMAIN )
		) );
	}
