...
class Fre_ProjectAction extends AE_PostAction {
	function __construct( $post_type = 'project' ) {
		...
		$this->add_ajax( 'ae-project-private', 'update_project_status' );
	...
	function update_project_status(){
		...
		if(!empty($request) &&!empty($request['ID'])){
			wp_update_post( array( 'ID' => $request['ID'], 'post_status' => 'private' ));

			wp_send_json( array(
				'success' => true,
				'permalink'=>get_permalink($request['ID']),
				'msg'     => __( "Update project successful! ", ET_DOMAIN )
			) );
		}else{
			wp_send_json( array(
				'success' => false,
				'msg'     => __( "Project not update successful! ", ET_DOMAIN )
			) );
		}
	...
	function post_sync() {
		...
		if ( ! is_wp_error( $result ) ) {
			...
			/* set custom code */
			if(!empty($request['custom_form']) && $request['custom_form'] == "custom_form"){
				$post_id = $result->ID;
				$category_id = (int)$request['et_t_id'];
				$taxonomy = 'project_type';
				$chek = wp_set_object_terms( $post_id, $category_id, $taxonomy,false);
				if(!empty($request['et_commite_min_hour'])){
					add_post_meta($post_id, 'et_commite_min_hour', $request['et_commite_min_hour']);
				}
				if(!empty($request['et_commite_max_hour'])){
					add_post_meta($post_id, 'et_commite_max_hour', $request['et_commite_max_hour']);
				}
			...
			/* Start */
		if($request['custom_form'] && $request['custom_form'] == "update_form" && $request['method'] == 'update'){
				$post_id = $_REQUEST['ID'];
				$category_id = (int)$_REQUEST['project_type'][0];
				$taxonomy = 'project_type';

				$chek = wp_set_object_terms( $post_id, $category_id, $taxonomy,false);
				
				if(!empty($request['et_commite_min_hour'])){
					update_post_meta($post_id, 'et_commite_min_hour', $request['et_commite_min_hour']);
				}
				if(!empty($request['et_commite_max_hour'])){
					update_post_meta($post_id, 'et_commite_max_hour', $request['et_commite_max_hour']);
				}	
			}else{
				$post_id = $_REQUEST['ID'];
				$category_id = (int)$_REQUEST['project_type'][0];
			...
			/*if($request['custom_form'] && $request['custom_form'] == "update_form_project" && $request['method'] == 'update'){
				$post_id = $_REQUEST['ID'];
				$category_id = (int)$_REQUEST['project_type'][0];
				$taxonomy = 'project_type';
	...
	function ae_convert_project( $result ) {
		...
		if ( $result->total_bids > 0 ) {
			if ( $result->total_bids == 1 ) {
				$result->text_total_bid = sprintf( __( '%s Offer', ET_DOMAIN ), $result->total_bids );
			} else {
				$result->text_total_bid = sprintf( __( '%s Offers', ET_DOMAIN ), $result->total_bids );
			}
		} else {
			$result->text_total_bid = sprintf( __( '%s Offers', ET_DOMAIN ), $result->total_bids );
		}
