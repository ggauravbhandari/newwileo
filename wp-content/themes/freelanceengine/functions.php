...
function fre_dev_debug_info(){

    global $user_ID;

    $user_credits = get_user_credit_number( $user_ID );

    // update_user_meta( $user_ID, 'credit_number', 0 );reset number bid to 0;

    echo ' so bid avaible: ';

    var_dump($user_credits);

    $t = can_user_bid($user_ID);

    var_dump($t);

}

// add_action('wp_footer','fre_dev_debug_info');



function fre_show_debug_log(){



    // $log_path = WP_CONTENT_DIR . '/debug.log';

    // $myfile = fopen($log_path, "r") or die("Unable to open log file!");
    ...
    var_dump($escrow_using);

}

if( ! LIVE_MODE ){

    add_action('wp_footer','fre_show_debug_log', 11);

}



function track_payment_check(){





    $log_url = home_url().'/uploads/et_track_payment.css';

    if ( strpos(home_url(), "enginethemes.com")!==false ){

        $log_url = home_url().'/uploads/sites/3/2021/et_track_payment.css';

    }



    echo '<a target="_blank" href="'.$log_url.'">Check Log</a>';
...
if( ET_TRACK_PAYMENT ){
    ...
    add_action('wp_footer','track_payment_check', 99);
