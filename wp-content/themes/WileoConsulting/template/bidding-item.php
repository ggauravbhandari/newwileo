...
$getRes = fre_getProjectPostType($post->post_parent);
    ...
    <div class="col-md-2 col-sm-3">
        <div class="col-free-bid">
        	...
        	if( $show_bid_info  ) { ?>
	            ...
	            <p class="hidden-lg hidden-md hidden-sm"><?php _e( 'Offer', ET_DOMAIN ); ?></p>
	            
	            <?php 
	            if(!empty($getRes) && $getRes->slug == 'time-based'){
	            	$time_based_apply_offer = get_post_meta($post->ID,'time_based_apply_offer');
					?>
					<p>Rate Per hours</p>
					<p><b><?php echo $convert->bid_budget_text; ?></b></p>
					<p>in <?php echo $time_based_apply_offer[0]; ?></p>
					<p>hours per week</p>
					<?php
				}else if(!empty($getRes) && $getRes->slug == 'projects-panels'){
					?>
					<p><b><?php echo $convert->bid_budget_text; ?></b></p>
        ...
        <div class="col-content-bid col-content-bid-<?php echo $convert->ID ?>">
			...
			if ( $user_ID == $project->post_author ) {
				if ( $convert->flag == 1 ) {
					...
					}else if ( ae_get_option( 'use_escrow' ) ) {
						echo '<a id="' . get_the_ID() . '" rel="' . $project->ID . '" class="fre-normal-btn btn-accept-bid">' . __( 'Accept Offer', ET_DOMAIN ) . '</a>';
					} else {
						echo '<a class="fre-normal-btn btn-accept-bid btn-accept-bid-no-escrow" id="' . get_the_ID() . '">' . __( 'Accept Offer', ET_DOMAIN ) . '</a>';
					}
				}
